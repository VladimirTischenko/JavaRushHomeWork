...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory{
        private static AtomicInteger countFactory = new AtomicInteger();
        private AtomicInteger countThread = new AtomicInteger();
        private int factoryNumber, threadNumber;
        private ThreadGroup threadGroup;

        public AmigoThreadFactory() {
            factoryNumber = countFactory.incrementAndGet();
            threadGroup = Thread.currentThread().getThreadGroup();
        }

        @Override
        public Thread newThread(Runnable r) {
            threadNumber = countThread.incrementAndGet();
            Thread thread = new Thread(r);
            thread.setDaemon(false);
            thread.setPriority(Thread.NORM_PRIORITY);
            thread.setName(threadGroup.getName() + "-pool-" + factoryNumber + "-thread-" + threadNumber);
            return thread;
        }
    }
}
